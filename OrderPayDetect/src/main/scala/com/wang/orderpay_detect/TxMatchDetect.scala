...
object TxMatchDetect {
  // 定义侧输出流tag
  // 1.订单支付存在数据，订单接收不存在数据
  val unmatchedPays = new OutputTag[OrderEvent]("unmachedPays")

  // 2.订单到账存在数据，订单支付不存在数据
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 将两条流连接起来共同处理
    val processedStream: DataStream[(OrderEvent, ReceiptEvent)] = orderEventStream.connect(recieptEventStream)
      .process(new TxPayMatch())

    processedStream.print("matched")
    processedStream.getSideOutput(unmatchedPays).print("unmachedPays")
